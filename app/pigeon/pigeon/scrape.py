...
def main():
    ...
    parser.add_argument("--max-articles", type=int, default=10, help="Maximum number of articles to scrape")
    parser.add_argument("--format", choices=["file", "kafka"], default="file", help="Storage format")
    parser.add_argument("--filename", help="Filename for file storage")
    parser.add_argument("--kafka-topic", default="articles", help="Kafka topic for kafka storage")
    ...
    # Save articles
    if args.format == "file":
        filename = save_articles(articles, format="file", filename=args.filename)
        ...
        print(f"Saved {len(articles)} articles to Kafka topic {args.kafka_topic}")
